...
def get_ventures(address):
    matrix = pd.read_csv(url2)

    # User input address
    addresstoAnalyze = address  # Analyzing the Catchment Area for the Starbucks Store (400, Pine Street, Seattle) in Westlake Center, Seattle.
    ...
    routeRangeResponse = requests.get(
        "https://atlas.microsoft.com/route/range/json?subscription-key={}&api-version=1.0&query={}&timeBudgetInSec={}&traffic={}&travelmode={}".format(
            subscriptionKey, str(currentLocation[0]) + "," + str(currentLocation[1]), walkingTime, cTraffic,
            tMode)).json()
    ...
    for n in range(106):
        ...
        for loc in range(len(searchPolyResponse["results"])):
            ...
            ventures.append([searchPolyResponse["results"][loc]["poi"]["name"], latitude, longitude])
    ...
    for venture in ventures:
        final_geojson_string += '''{ "type": "Feature", "properties":{"name":"''' + venture[
            0] + '''"}, "geometry": { "type": "Point", "coordinates": [ ''' + str(venture[2]) + ''', ''' + str(
            venture[1]) + ''' ] } },'''
...
get_ventures("Ružová 734, 040 11 Košice")
